...
kospi = pd.read_sql("""select trd_date, gicode, co_nm,iskosdaq, wics_mid,cap_size,EQUITY,CFO_TTM,cash_div_com,SALES_TTM,ADJ_NI_12M_FWD,market_cap,adj_prc, asset,asset + 0.1*(market_cap/1000 -equity) as adjasset,asset - equity as liab,ni,liq_equity,liq_debt,(NI-lag(ni,3) over(partition by gicode order by trd_date))/ (ABS(ni) + abs(lag(ni,3) over(partition by gicode order by trd_date))) as chin,lag(ni,12) over(partition by gicode order by trd_date) as ni_1y, gross_profit_ttm ,
                    (gross_profit_ttm-lag(gross_profit_ttm,36) over(partition by gicode order by trd_date))/lag(asset,36) over(partition by gicode order by trd_date) as D3GPOA,
                    (ni-lag(ni,36) over(partition by gicode order by trd_date))/lag(equity,36) over(partition by gicode order by trd_date) as D3ROE,
                    (ni-lag(ni,36) over(partition by gicode order by trd_date))/lag(asset,36) over(partition by gicode order by trd_date) as D3ROA,
                    (cfo_ttm-lag(cfo_ttm,36) over(partition by gicode order by trd_date))/lag(asset,36) over(partition by gicode order by trd_date) as D3CFOA,
                    (gross_profit_ttm-lag(gross_profit_ttm,36) over(partition by gicode order by trd_date))/lag(sales_ttm,36) over(partition by gicode order by trd_date) as D3GMAR,
                    (ni -lag(ni,36) over(partition by gicode order by trd_date)- cfo_ttm+ lag(cfo_ttm,36) over(partition by gicode order by trd_date))/lag(asset,36) over(partition by gicode order by trd_date) as D3ACC,
...
gross_col_loc = ['D3GPOA','D3ROE','D3ROA','D3CFOA','D3GMAR','D3ACC']
col1 = []
...
for i in factors:
    a=factor_1_mid_adoutlier(50,raw_data,rebalancing_date,month_date,wics_mid,daily_return,i)
    locals()['aaa_{}'.format(i)] =a.factor_1_mid_adoutlier()
    b=result_calculator(locals()['aaa_{}'.format(i)][0],kospi_day)
    b.rolling_12month_return_1factor(i)
...
for i in factors:
    for j in factors:
        for z in factors:
            if factors[factors==i].index[0]<factors[factors==j].index[0]<factors[factors==z].index[0]:
                a=factor_3_mid_adoutlier_pca(50,raw_data,rebalancing_date,month_date,wics_mid,daily_return,i,j,z)
                locals()['aaa_pca_{}{}{}'.format(i,j,z)] =a.factor_3_mid_adoutlier_pca()
                b=result_calculator(locals()['aaa_pca_{}{}{}'.format(i,j,z)][0],kospi_day)
                b.rolling_12month_return_3factor(i,j,z)
...
for i in factors:
    for j in factors:
        for z in factors:
            for p in factors:
                if factors[factors==i].index[0]<factors[factors==j].index[0]<factors[factors==z].index[0]<factors[factors==p].index[0]:
                    a=factor_4_mid_adoutlier(50,raw_data,rebalancing_date,month_date,wics_mid,daily_return,i,j,z,p)
                    locals()['aaa_{}{}{}{}'.format(i,j,z,p)] =a.factor_4_mid_adoutlier()
                    b=result_calculator(locals()['aaa_{}{}{}{}'.format(i,j,z,p)][0],kospi_day)
...
for i in factors:
    for j in factors:
        for z in factors:
            for p in factors:
                for k in factors:
                    if factors[factors==i].index[0]<factors[factors==j].index[0]<factors[factors==z].index[0]<factors[factors==p].index[0]<factors[factors==k].index[0]:
                        a=factor_5_mid_adoutlier(50,raw_data,rebalancing_date,month_date,wics_mid,daily_return,i,j,z,p,k)
                        locals()['aaa_{}{}{}{}{}'.format(i,j,z,p,k)] =a.factor_5_mid_adoutlier()
                        b=result_calculator(locals()['aaa_{}{}{}{}{}'.format(i,j,z,p,k)][0],kospi_day)
    ...
    for j in factors:
        for z in factors:
            for p in factors:
                for k in factors:
                    for u in factors:
                        if factors[factors==i].index[0]<factors[factors==j].index[0]<factors[factors==z].index[0]<factors[factors==p].index[0]<factors[factors==k].index[0]<factors[factors==u].index[0]:
                            a=factor_6_mid_adoutlier(50,raw_data,rebalancing_date,month_date,wics_mid,daily_return,i,j,z,p,k,u)
                            locals()['aaa_{}{}{}{}{}{}'.format(i,j,z,p,k,u)] =a.factor_6_mid_adoutlier()
                            b=result_calculator(locals()['aaa_{}{}{}{}{}{}'.format(i,j,z,p,k,u)][0],kospi_day)
                            b.rolling_12month_return_6factor(i,j,z,p,k,u)
        ...
        for z in factors:
            for p in factors:
                for k in factors:
                    for u in factors:
                        if factors[factors==i].index[0]<factors[factors==j].index[0]<factors[factors==z].index[0]<factors[factors==p].index[0]<factors[factors==k].index[0]<factors[factors==u].index[0]:
                            a=factor_6_mid_adoutlier_pca(50,raw_data,rebalancing_date,month_date,wics_mid,daily_return,i,j,z,p,k,u)
                            locals()['aaa_{}{}{}{}{}{}'.format(i,j,z,p,k,u)] =a.factor_6_mid_adoutlier_pca()
                            b=result_calculator(locals()['aaa_{}{}{}{}{}{}'.format(i,j,z,p,k,u)][0],kospi_day)
                            b.rolling_12month_return_6factor(i,j,z,p,k,u)
                
        
#모든  팩터.. 
a=factor_whole_mid_fac_ad(50,raw_data,rebalancing_date,month_date,wics_mid,daily_return,factors)
aaa_whole =a.factor_whole_mid_fac_ad()
b=result_calculator(aaa_whole[0],kospi_day)
b.rolling_12month_return_whole_factor(factors)

#모든  팩터.. pca
a=factor_whole_mid_fac_ad_pca(50,raw_data,rebalancing_date,month_date,wics_mid,daily_return,factors)
aaa_whole =a.factor_whole_mid_fac_ad_pca()
b=result_calculator(aaa_whole[0],kospi_day)
b.rolling_12month_return_whole_factor(factors)
