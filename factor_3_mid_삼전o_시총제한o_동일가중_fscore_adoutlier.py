...
29개의 wics 섹터 중에서 1년 - 1개월 누적수익률 상위 15섹터를 고른 후에, f_score을 계산하고
전체 유니버스에 대해서 각 팩터 z_score을 구하고, 섹터별 평균을 뺸다음 합친다.
...
#                
            ...
            locals()['data_{}'.format(0)] = locals()['data_{}'.format(0)].assign(z_score=np.nanmean(locals()['data_{}'.format(0)].loc[:,self.col_loc],axis=1))
